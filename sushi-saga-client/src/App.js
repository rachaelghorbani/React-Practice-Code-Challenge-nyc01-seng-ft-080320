...
class App extends Component {
	...
	moreClickHandler = () => {
		if (this.state.minReturnedId > this.state.api.length - 4) {
			this.setState({
				minReturnedId: 1,
				maxReturnedId: 4
			});
...
*/
	...
	componentDidMount() {
		fetch(API).then((resp) => resp.json()).then((sushis) => {
		...
		return (
			<div className="app">
				<SushiContainer
					...
					minReturnedId={this.state.minReturnedId}
					maxReturnedId={this.state.maxReturnedId}
					clickHandler={this.moreClickHandler}
